...
int main(int argc, char **argv) {
    ros::init(argc, argv, "ps4_j2_commander"); // name of this node will be "minimal_publisher2"
    ros::NodeHandle n; // two lines to create a publisher object that can talk to ROS
    ros::Publisher my_publisher_object = n.advertise<std_msgs::Float64>("ps4_j2_pos_cmd", 1);
   ...
   //set the sleep timer for 1Hz repetition rate (arg is in units of Hz)
    ...
    while (ros::ok()) 
    {
        ...
        input_float.data = A.data*sin(omega.data*t)-0.2; //increment by 0.001 each iteration
