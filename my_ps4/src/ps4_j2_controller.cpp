...
int main(int argc, char **argv) {
    ros::init(argc, argv, "ps4_j2_controller");
    ros::NodeHandle nh;
    ...
    ros::Publisher trq_publisher = nh.advertise<std_msgs::Float64>("ps4_j2_jnt_trq", 1); 
    ros::Publisher vel_publisher = nh.advertise<std_msgs::Float64>("ps4_j2_jnt_vel", 1);     
    ros::Publisher pos_publisher = nh.advertise<std_msgs::Float64>("ps4_j2_jnt_pos", 1);  
    ros::Publisher joint_state_publisher = nh.advertise<sensor_msgs::JointState>("ps4_j2_joint_states", 1); 

    ros::Subscriber pos_cmd_subscriber = nh.subscribe("ps4_j2_pos_cmd",1,posCmdCB); 
    ...
    effort_cmd_srv_msg.request.joint_name = "ps4_j2";
    effort_cmd_srv_msg.request.effort = 0.0;
    effort_cmd_srv_msg.request.duration= duration;

    get_joint_state_srv_msg.request.joint_name = "ps4_j2";
    //double q1_des = 1.0;
    double q1_err;
    double Kp = 10.0;
    double Kv = 3;
	...
	joint_state_msg.name.push_back("ps4_j2");
